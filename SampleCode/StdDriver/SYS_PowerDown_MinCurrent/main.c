...
void GPAB_IRQHandler(void)
{
    ...
    {
        /* Un-expected interrupt. Just clear all PortA and PortB interrupts */
        PA->INTSRC = PA->INTSRC;
        PB->INTSRC = PB->INTSRC;
...
void SYS_Init(void)
{
    ...
    /* Set PD multi-function pins for UART0 RXD (PD.9) and TXD (PD.1) */
    SYS->GPD_MFPL = (SYS->GPD_MFPL & (~SYS_GPD_MFPL_PD1MFP_Msk)) | SYS_GPD_MFPL_PD1MFP_UART0_TXD;
    SYS->GPD_MFPH = (SYS->GPD_MFPH & ~(SYS_GPD_MFPH_PD9MFP_Msk)) | SYS_GPD_MFPH_PD9MFP_UART0_RXD;
